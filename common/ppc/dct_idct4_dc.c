...
#define ALTIVEC_STORE4_DC_SUM_CLIP(dest, dcv)                    \
{                                                               \
    /* unaligned load */                                        \
    vec_u8_t dstv = vec_vsx_ld(0, dest);                        \
    vec_u16_t dst16 = vec_u8_to_u16_h(dstv);                    \
    vec_s16_t dcsum = vec_adds(dcv, (vec_s16_t)dst16);   \
    vec_u8_t dcsum8 = vec_s16_to_u8(dcsum);                 \
    /* unaligned store */                                       \
    vec_u32_t bodyv = vec_splat((vec_u32_t)dcsum8, 0);        \
    int element = ((unsigned long)dest & 0xf) >> 2;             \
    vec_ste(bodyv, element, (uint32_t *)dest);                  \
}
void x264_add4x4_idct_dc_altivec( uint8_t *dst, int16_t dc )
{
    dc = (dc + 32) >> 6;
    vec_s16_t dcv = {dc, dc, dc, dc, dc, dc, dc, dc};

    LOAD_ZERO;
    ALTIVEC_STORE4_DC_SUM_CLIP( &dst[0*FDEC_STRIDE], dcv);
    ALTIVEC_STORE4_DC_SUM_CLIP( &dst[1*FDEC_STRIDE], dcv);
    ALTIVEC_STORE4_DC_SUM_CLIP( &dst[2*FDEC_STRIDE], dcv);
    ALTIVEC_STORE4_DC_SUM_CLIP( &dst[3*FDEC_STRIDE], dcv);
}
