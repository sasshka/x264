...
static int validate_parameters( x264_t *h, int b_open )
{
    ...
    else if( CHROMA_FORMAT != CHROMA_420 && i_csp >= X264_CSP_I420 && i_csp <= X264_CSP_NV21 )
    {
    ...
    if( i_csp == X264_CSP_I400 )
    {
        ...
        h->param.vui.i_colorprim = 2; /* undefined */
        h->param.vui.i_transfer = 2;
...
x264_t *x264_encoder_open( x264_param_t *param )
{
    ...
    *h->reconfig_h = *h;
    ...
    const char *profile = h->sps->i_profile_idc == PROFILE_BASELINE ? "Constrained Baseline" :
                          ...
                          (h->sps->b_constraint_set4 == 1 ? (h->sps->b_constraint_set5 == 1 ? "Constrained High" : "Progressive High") : "High") :
                          h->sps->i_profile_idc == PROFILE_HIGH10 ?
                          (h->sps->b_constraint_set3 == 1 ? "High 10 Intra" : (h->sps->b_constraint_set4 ? "Progressive High 10" : "High 10")) :
    ...
    static const char subsampling[4][6] = { "4:0:0", "4:2:0", "4:2:2", "4:4:4" };
